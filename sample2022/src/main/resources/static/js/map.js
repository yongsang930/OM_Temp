...
GCRM.fn.getWms = function() {
	...
	if (!tf_wms) {
		...
		var wmsLayer = L.tileLayer.wms('http://localhost:8088/geoserver/test/wms', {
			// geoserver version
			version: '1.1.1',
...
GCRM.fn.getWfs = function() {
	...
	// 객체를 string으로 변환
	var URL = 'http://localhost:8088/geoserver/test/ows/service=WFS' + L.Util.getParamString(parameters) + decode;
...
GCRM.fn.getWfsUMD = function() {
	...
	var defaultParameters = {
		...
		request: 'GetFeature',
		// wfs의 위치
		typeName: 'test:Seoul_filter_wfs',
		// 결과를 얻을 피처 수로 지정하지 않으면 모두 반환
		// maxFeature: 50,
		// output의 포맷은 json
		outputFormat: 'application/json',
	...
	var filter = "&viewparams=cdName:11";
	var decode = encodeURI(filter);
	...
	function clickcEventUMD(e) {
		...
		//	GCRM.MAP.fitBounds(e.target.getBounds());
		...
		var ajax = $.ajax({
			...
			success: function(data) {
				...
				var wfsLayer = L.geoJSON(data, {
					...
					style: offStyle,
					onEachFeature: function(feature, layer) {
						//feature._gubun = "getWfsUMD";
						//layer._name = "getWfsUMD";
						var UMDName = feature.properties.emd_kor_nm;
						var UMDCode = feature.properties.emd_cd;
						var popupContent = '<div id="UMDPopup">' + '- 읍면동: ' + UMDName + '</br>' + '- 코드: ' + UMDCode + '</div>';

						layer.bindPopup(popupContent);

						layer.on({
							click: clickcEventUMD
						});
					}
				}).addTo(map);
